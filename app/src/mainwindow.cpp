...
MainWindow::MainWindow(QWidget *parent)
  ...
  ui->port_selector->addItem(usb_ubuntu);
  ...
  if (!initialized) {
    ...
    if (connection_result != ConnectionResult::Success) {
      ...
      if (!system) {
        ...
        ////////////////////////////////////////////////////// PASCAL
        // subscribe to battery and flightmode informations
        telemetry->subscribe_battery(
          [&](Telemetry::Battery battery) {
            // battery update in %
            QString percentage = QString::number(battery.remaining_percent*100);
            percentage.append("%");
            ui->battery_remainingPercent_lb->setText(percentage);
            //flight mode update
            auto flightMode = telemetry->flight_mode();
            if(flightMode == Telemetry::FlightMode::Offboard) {
              ui->flightMode_lb->setText("Offboard control");
            }else if(flightMode == Telemetry::FlightMode::Manual) {
              ui->flightMode_lb->setText("Manual control");
            }else if(flightMode == Telemetry::FlightMode::Hold) {
              ui->flightMode_lb->setText("Holding position");
            }else if(flightMode == Telemetry::FlightMode::Ready) {
              ui->flightMode_lb->setText("Ready");
            }else if(flightMode == Telemetry::FlightMode::Takeoff) {
              ui->flightMode_lb->setText("Taking off");
            }else if(flightMode == Telemetry::FlightMode::Land) {
              ui->flightMode_lb->setText("Landing");
            }
          }
        );
        ////////////////////////////////////////////////////// PASCAL END
